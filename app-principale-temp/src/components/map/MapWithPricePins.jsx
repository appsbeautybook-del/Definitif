        ...
        return (
          <div className="absolute bottom-3 left-3 right-3 bg-white rounded-2xl shadow-lg px-4 py-3 z-[1000] flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[13px] font-semibold text-gray-900 truncate">{item.title || item.name}</p>
              <p className="text-[11px] text-gray-500 truncate">{item.address || item.city || item.location}</p>
            </div>
            {item.price > 0 && <span className="text-[15px] font-semibold text-gray-900 shrink-0">{item.price}€</span>}
          </div>
        );
      })()}
    </div>

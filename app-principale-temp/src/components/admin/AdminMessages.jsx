...
export default function AdminMessages() {
  ...
  const load = async () => {
    try {
      ...
      const res = await adminApi.filterMessages({ receiver_email: ADMIN_EMAIL }, "-created_at", 200).catch(() => []);
      const msgs = Array.isArray(res) ? res : res?.data?.results || [];
      ...
      const sentRes = await adminApi.filterMessages({ sender_email: ADMIN_EMAIL }, "-created_at", 100).catch(() => []);
      const sentMsgs = Array.isArray(sentRes) ? sentRes : sentRes?.data?.results || [];
  ...
  const loadConversation = async (conv) => {
    ...
    try {
      ...
      const convRes = await adminApi.filterMessages({ conversation_id: conv.id }, "created_at", 100).catch(() => []);
      const all = Array.isArray(convRes) ? convRes : convRes?.data?.results || [];
      setMessages(all);

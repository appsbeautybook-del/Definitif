...
function CabineEssayage({ products, likedProducts, preSelectedProduct }) {
  ...
  const analyzePhoto = async (photoUrl, productName) => {
...
Respond in JSON only.`,
  ...
  const handleUserPhoto = async (e) => {
    ...
    e.target.value = "";
    analyzePhoto(url, selectedProduct?.name || "");
  };
...
export default function ShAI() {
  ...
  useEffect(() => {
    Promise.allSettled([
      ...
      supabase.from('Produit').select('id,name,brand,price,image_url').eq('status', 'actif').order('created_at', { ascending: false }).limit(200)
        .then(({ data }) => (data || []).map(p => ({ id: p.id, name: p.name, brand: p.brand || "", price: p.price, img: p.image_url || "" }))),
    ]).then(([shopify, db]) => {
      const all = [

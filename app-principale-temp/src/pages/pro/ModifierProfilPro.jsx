...
export default function ModifierProfilPro() {
  ...
  useEffect(() => {
    ...
    supabase.from('ProfilPro').select('id, user_email, salon_name, phone, address, city, bio, avatar_url, cover_url, galerie_urls, horaires, ouverture, seats_count, specialites, commodites, galerie_urls').eq('user_email', user.email).maybeSingle()
      .then(({ data: p }) => {
        let profile = p;
        if (!profile) {
          try {

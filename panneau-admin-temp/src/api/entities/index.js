...
const parseOrder = (orderBy) => {
  ...
  const colMap = {
    ...
    date: 'date', rating: 'rating', price: 'price', likes: 'likes', views: 'views',
...
const KNOWN_COLUMNS = {
  ...
  Style: ['id','title','description','images','video_url','category','tags','status','likes','views','author_email','author_name','author_avatar','pro_email','service_id','price','duration_min','featured','is_sponsored','created_at','updated_at','created_by_id'],
  ...
  Reservation: ['id','user_email','service_id','service_name','pro_email','date','time','duration','status','price','notes','created_at','updated_at','created_by_id'],
  Avis: ['id','user_email','pro_email','service_id','rating','comment','status','created_at','updated_at'],
  AppConfig: ['id','key','value','description','created_at','updated_at'],
  CallLog: ['id','caller_email','callee_email','started_at','ended_at','duration','status','created_at'],
  ImmobilierListing: ['id','title','description','price','address','city','type','status','images','pro_email','created_at','updated_at'],
  Panier: ['id','user_email','items','total','status','created_at','updated_at'],
  UserMemory: ['id','user_email','key','value','created_at','updated_at'],
  VerificationCode: ['id','email','code','expires_at','created_at'],
  VisiteVirtuelle: ['id','title','scenes','pro_email','status','created_at','updated_at'],
  MariaConversation: ['id','user_email','messages','created_at','updated_at'],
  Repub: ['id','reel_id','user_email','created_at'],
};
...
const createEntity = (tableName) => ({
  ...
  update: async (id, data) => {
    try {
      ...
      const payload = { ...data };
      Object.keys(payload).forEach(k => payload[k] === undefined && delete payload[k]);
      const { data: result, error } = await supabase.from(tableName).update(payload).eq('id', id).select().single();
      if (error) throw error;
      return { data: { [tableName.toLowerCase()]: result }, result };
    }
  },
...
export const uploadFile = async (fileOrObj, bucket = 'uploads') => {
  ...
  const { data, error } = await supabase.storage
    .from(bucket)
    .upload(filePath, rawFile, { contentType: rawFile.type, upsert: true });

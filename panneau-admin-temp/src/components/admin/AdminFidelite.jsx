...
export default function AdminFidelite() {
  ...
  const load = async () => {
    ...
    if (cli.status === "fulfilled") {
      const { data } = cli.value;
      setClientRecords(data?.results || []);
    }
    if (pro.status === "fulfilled") {
      const { data } = pro.value;
      setProRecords(data?.results || []);
    }
  ...
  const saveConfig = async (type, data) => {
    const { data: cfgData } = await adminApi.getConfig("fidelite_config").catch(() => ({ data: { results: [] } }));
    const rows = cfgData?.results || [];

...
function StylesTab() {
  ...
  useEffect(() => {
    adminApi.listStyles()
      .then(({ data }) => setStyles(data?.results || [])).catch(() => {}).finally(() => setLoading(false));
  }, []);
...
function ServicesTab() {
  ...
  useEffect(() => {
    adminApi.listServices().then(({ data }) => setServices(data?.results || [])).catch(() => {}).finally(() => setLoading(false));
  }, []);
...
function SalonsTab() {
  ...
  useEffect(() => {
    adminApi.listProfilsPro().then(({ data }) => setSalons(data?.results || [])).catch(() => {}).finally(() => setLoading(false));
  }, []);
...
function ParticuliersTab() {
  ...
  useEffect(() => {
    entities.ProfilPro.filter({ type_activite: "Particulier" }, "-created_at", 100)
      .then(({ data }) => setPros(data?.results || []))
      .catch(() => adminApi.listProfilsPro().then(({ data }) => setPros(data?.results || [])))
...
function RecommandesTab() {
  ...
  useEffect(() => {
    entities.ProfilPro.filter({ status: "actif" }, "-created_at", 100)
      .then(({ data }) => setSalons(data?.results || [])).catch(() => {}).finally(() => setLoading(false));
  }, []);
    ...
    <div className="space-y-4">
      ...
      {experiences.map((exp, i) => (
        <div key={exp.id || i} className="bg-white rounded-2xl p-4 border border-gray-200 shadow-sm space-y-3">
          ...
          <input value={exp.image_url} onChange={e => update(i, "image_url", e.target.value)} placeholder="URL de l'image" className={inputCls} />
